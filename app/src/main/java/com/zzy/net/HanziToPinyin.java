...
public class HanziToPinyin {
    ...
    private static boolean bFoundChina =false;
...
//                        + "\" is greater than current string \"" + curString + "\".");
    ...
    private static String getChar(char c, StringBuffer result)
    {
		if((c>='A' && c<='Z') || (c>='a' && c<='z'))
		{
			result.append(c);
    ...
    {
		List<Locale> lsLocales = Arrays.asList(Collator.getAvailableLocales());
		boolean bFound = false;
		for(Locale locale: lsLocales)
		{
...
//		{
//			return getChar(source.charAt(0),result);
//		}
		...
		if(!bFoundChina)
		{
			if(IsAlph(source.charAt(0)))
			{
				return source.toUpperCase(Locale.US); //getChar(source.charAt(0),result);
			}
			else
			{
				return "#";
		...
		{
			//c = token.target.charAt(0);
//			if (token.type == Token.PINYIN)
//			{
//				return result.append(token.target);
//			}
//			else
//			{
//				return getChar(c,result);
//			}
			result.append(token.target);
		}
		
		String sTmp = result.toString();
		if(IsAlph(sTmp.charAt(0)))
		{
			return sTmp.toUpperCase(Locale.US); //getChar(source.charAt(0),result);
		}
		else
		{
			return "#";
